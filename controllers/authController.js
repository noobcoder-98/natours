...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('Incorrect email. Please try again!'))
  }
...
exports.logout = catchAsync(async (req, res, next) => {
  console.log('da vao logout func')
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  const currentUser = await User.findById(decoded.id)
  if (!currentUser) {
    return next(new AppError('The user belonging to this token does not no longer exist'), 401)
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm
  await user.save()
  // CAUTION: User.findByIdAndUpdate will not word as intented
